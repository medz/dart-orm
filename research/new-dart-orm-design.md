...
一套应用迁移历史固定一个数据库引擎，包括空注册表。单个迁移只有该引擎的一组步骤、转换 SQL 和历史物理 schema；ORM 的多库编译能力不进入应用的迁移文件。共享声明的另一引擎配置不影响当前历史的 diff 或指纹。多库应用分别维护目录、注册表和连接；换库属于数据搬迁及新库 baseline，不能重放或改写原库历史。生产运行器只执行已审阅迁移。Flutter 本地数据库可以在业务开始前应用打包的迁移，服务端则建议作为独立发布步骤。
